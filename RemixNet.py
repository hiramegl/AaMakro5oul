...
class OSCServer:
    ...
    def __init__(self, parent, dst=None, dstPort=None, src=None, srcPort=None, ):
        """
        ...
        self.udpClient = UDPClient(parent, dst, dstPort)
        self.udpClient.open()
        ...
        self.udpServer = UDPServer(parent, self.src_host, self.src_port)
    ...
    def try_server(self):
        if not self.udpServer.is_bound():
            self.parent.log('> OSCServer - Binding UDP port')
            self.udpServer.bind()
    ...
    def sendBundle(self, messages):
        if len(messages) > 99:
            self.doSendBundle(messages[0:99])
            self.doSendBundle(messages[99:])
        else:
            self.doSendBundle(messages)


    def doSendBundle(self, messages):
        if len(messages) > 0:
            ...
            for message in messages:
                msg = OSC.OSCMessage()
                msg.setAddress(message[0])

                if type (message[1]) == type([]):
...
class UDPClient:
    ...
    def __init__(self, parent, dst=None, dstPort=None):
        """
    ...
    def send(self, data):
        ...
        if not data == '':
            try:
                ...
                self.parent.c_instance.show_message('ERROR: Could not send outgoing data, check your network settings')
                self._outgoing = 0
...
class UDPServer:
    ...
    def __init__(self, parent, src, srcPort):
        """
        ...
        try:
            ...
            while 1:
                ...
                # No data buffered this round!
                    ...
                    #Live.Base.log("-> Rx: '{0}'".format(self.data))
                    if self.data != '\n':
    ...
    def bind(self):
        ...
        try:
            ...
            self.bound = 1
            self.parent.c_instance.show_message('LiveControl Ready!')
        except:
            self.bound = 0
            self.parent.c_instance.show_message('ERROR: Could not bind to incoming ip/port, check your LiveControl network settings')
